...
Output JSON format:
{
  ...
  debug: `You are an expert code debugger and quality assurance specialist. Analyze the provided code to find errors, potential bugs, and suggest fixes. Provide corrected code when possible.
...
router.post('/debug', optionalAuth, async (req, res) => {
  try {
    console.log('🐛 DEBUG route called with Gemini API');
    const { code, language } = req.body;
    ...
    console.log('🚀 Calling Gemini API for code debugging...');
    const result = await callGeminiDebug(code, language);
